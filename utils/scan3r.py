...
@lru_cache(maxsize=None)
def scan_contains_object(data_dir: str, scan: str, obj: Tuple[str]) -> bool:
    objects = read_objects(data_dir)
    objects_in_scan = next(
        (item["objects"] for item in objects["scans"] if item["scan"] == scan), None
    )
    if objects_in_scan is None:
        return False
    return any(o["global_id"] in obj for o in objects_in_scan)


def is_anchor_object(
    data_dir: str, scan: str, obj: Tuple[str], anchor_data: dict
) -> bool:
    objects = read_objects(data_dir)
    anchor_objects_id = anchor_data["anchorIds"]
    objects_in_scan = next(
        (item["objects"] for item in objects["scans"] if item["scan"] == scan), None
    )
    local_id_to_global_id = {int(o["id"]): o["global_id"] for o in objects_in_scan}
    anchor_objects_global_id = [
        local_id_to_global_id[local_id]
        for local_id in anchor_objects_id
        if local_id in local_id_to_global_id
    ]
    return any(o in anchor_objects_global_id for o in obj)
...
def load_intrinsics(data_dir, scan_id, type="color"):
    ...
    calibration_search_string = (
        ...
        if type == "color"
        else "m_calibrationDepthIntrinsic"
    )
...
def load_patch_feature_scans(data_root_dir, feature_folder, scan_id, skip=None):
    ...
    features_file = osp.join(
        osp.join(data_root_dir, "files"), feature_folder, scan_id + ".pkl"
    )
...
def sample_candidate_scenes(
    ...
    # filter out the scans to be removed
    if num_scenes < 0:
...
def create_ply_data_predicted(ply_data, visible_pts_idx):
    ...
    vertices = np.empty(
        len(visible_pts_idx),
        dtype=[("x", "f4"), ("y", "f4"), ("z", "f4"), ("objectId", "h")],
    )
    vertices["x"] = x.astype("f4")
    vertices["y"] = y.astype("f4")
    vertices["z"] = z.astype("f4")
...
def depthmap2pc(depth_map, intrinsic, depth_range):
    ...
    pc_valid = pc_valid[np.isnan(pc_valid).any(axis=1) is False]
    return pc_valid
...
def load_scan_depth_pcs(data_dir, scan_id, scale, intrinsic, depth_range, step=1):
    ...
    for frame_idx, depth_path in depth_paths.items():
        ...
        pcs_valid = pcs_valid[np.isnan(pcs_valid).any(axis=1) is False]
...
def load_scan_pcs(
    data_dir, scan_id, transform_rescan2ref, ref_coord=False, color=False
):
...
def createRangeImage(pcs, colors, center, fov_up, fov_down, proj_W, proj_H, range):
    ...
    proj_range = np.full(
        (proj_H, proj_W, 3), 0, dtype=np.uint8
    )  # [H,W] range (-1 is no data)
    proj_color = np.full(
        (proj_H, proj_W, 3), 0, dtype=np.uint8
    )  # [H,W] index (0 is no data)
...
def loadScanMeshRange(
    mesh_file, fov_up, fov_down, range_min, range_max, range_H, range_W
):
    import open3d as o3d
    ...
    pitchs = np.linspace(
        fov_up * degree_to_radian, fov_down * degree_to_radian, range_H, endpoint=False
    )
    ...
    np.arange(len(trian_ids))[valid_mask]
...
def get_scans_id(data_item: Dict[str, Any]) -> np.array:
    start_obj_idx = 0
    scene_ids = []
    for idx, scan_id in enumerate(data_item["scene_graphs"]["scene_ids"]):
        scan_id = scan_id[0]
        graph_per_obj_count = data_item["scene_graphs"]["graph_per_obj_count"][idx][0]
        scene_ids += [scan_id for _ in range(graph_per_obj_count)]
        start_obj_idx += graph_per_obj_count
    scene_ids = np.array(scene_ids).flatten()
    return scene_ids
...
def load_mesh(data_dir: str, scan_id: str, load_seg: bool):
    ...
    if load_seg:
        seg_path = osp.join(data_dir, "scenes", scan_id, "semseg.v2.json")
        segmentation = json.load(open(seg_path))
