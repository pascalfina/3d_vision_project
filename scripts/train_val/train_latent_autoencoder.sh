...
export VLSG_TRAINING_OUT_DIR="$SCRATCH/training_voxel/$timestamp"

# Navigate to VLSG space
cd "$VLSG_SPACE" || { echo "Failed to change directory to $VLSG_SPACE"; exit 1; }

# Run training script
python src/trainval/train_latent_autoencoder.py --config scripts/train_val/train_latent_autoencoder.yaml --log_steps 1 output_dir=\"$VLSG_TRAINING_OUT_DIR\" ${args[@]}
