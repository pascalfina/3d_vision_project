...
class Gaussian:
    def __init__(
        ...
        aabb: list,
        sh_degree: int = 0,
        mininum_kernel_size: float = 9e-4,
    ...
    def load_ply(self, path):
        ...
        self._scaling = (
            self.inverse_scaling_activation(
                torch.sqrt(
                    torch.clamp(
                        torch.square(scales) - self.mininum_kernel_size**2, min=1e-9
                    )
                )
            )
            - self.scale_bias
    ...
    def rescale(self, scaling):
        ...
        self._scaling += self.inverse_scaling_activation(
            torch.tensor(scaling, device=self.device)
        )
    ...
    def to_pt(self):
        ...
        sh0 = self._features_dc.transpose(1, 2).flatten(start_dim=1).contiguous()
        opacities = inverse_sigmoid(self.get_opacity)
        scales = torch.log(self.get_scaling)
        quats = self._rotation + self.rots_bias[None, :]
        ...
        if f_rest is None:
            f_rest = torch.zeros(
                (sh0.shape[0], 15, 3), dtype=sh0.dtype, device=sh0.device
            )
        else:
