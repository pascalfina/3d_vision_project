#!/usr/bin/env python3
"""Generate lightweight ScanNet MUSt3R-only geometry profiles from a scene TSV."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate ScanNet MUSt3R-only scene profiles.")
    parser.add_argument(
        "--scene-table",
        type=Path,
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("configs/workflows/scene_profiles/scannet_must3r_under300_profiles.txt"),
    )
    parser.add_argument("--max-scenes", type=int, default=0)
    parser.add_argument(
        "--prune-stale",
        action="store_true",
        help="Remove generated scannet_scene*_must3r.json profiles not in the current manifest.",
    )
...
def profile_name(scene_id: str) -> str:
    return f"scannet_{scene_id}_must3r"
...
def make_profile(scene_id: str, frame_count: int) -> dict:
    ...
    profile = {
        ...
        "geometry_eval": {
            ...
            "log": f"${{OBJECTX_WORKFLOW_LOG_ROOT}}/scannet_{short}_must3r_geometry_eval.log",
        },
    }
...
def main() -> None:
    ...
    for scene_id, frame_count in scenes:
        ...
        if not args.dry_run:
            path.write_text(
                json.dumps(make_profile(scene_id, frame_count), indent=2) + "\n",
                encoding="utf-8",
            )
    ...
    if not args.dry_run:
        ...
        if args.prune_stale:
            ...
            for path in args.profile_dir.glob("scannet_scene*_must3r.json"):
                if path.name not in keep:
                    path.unlink()

    action = "would write" if args.dry_run else "wrote"
    print(f"[scannet-must3r-profiles] {action} {len(manifest_names)} profiles")
    print(f"[scannet-must3r-profiles] manifest={args.manifest}")
