...
# ScanNet geometry benchmark matching the ScanNet Pi3X sequence benchmark,
# but evaluating MUSt3R poses/depth directly without any Pi3X pass.
#
# Per scene:
#   1. select/download/export ScanNet scenes with <= SCANNET_SELECTION_MAX_FRAMES
#   2. run MUSt3R for poses + depth
#   3. evaluate the MUSt3R sequence geometry against ScanNet GT mesh
#   4. clean generated Object-X artifacts and optionally purge prepared ScanNet data
...
PROFILE_LIST="${PROFILE_LIST:-$REPO_ROOT/configs/workflows/scene_profiles/scannet_must3r_under300_profiles.txt}"
GEOMETRY_EVAL_GROUP="${GEOMETRY_EVAL_GROUP:-scannet_must3r_sequence_under300}"
...
echo "[scannet-must3r-benchmark] root=$SCANNET_ROOT"
echo "[scannet-must3r-benchmark] work_root=$SCANNET_WORK_ROOT"
echo "[scannet-must3r-benchmark] target=$SCANNET_TARGET_SCENES max_frames=$SCANNET_SELECTION_MAX_FRAMES mode=$SCANNET_SELECTION_MODE order=$SCANNET_SELECTION_ORDER group=$GEOMETRY_EVAL_GROUP"
echo "[scannet-must3r-benchmark] force=$FORCE skip_completed=$SKIP_COMPLETED cleanup=$CLEANUP_AFTER_SCENE continue=$CONTINUE_ON_ERROR dry_run=$DRY_RUN reuse_selection=$SCANNET_REUSE_SELECTION prune_profiles=$SCANNET_PRUNE_STALE_PROFILES"
echo "[scannet-must3r-benchmark] purge_prepared_after_scene=$SCANNET_PURGE_PREPARED_AFTER_SCENE purge_failed=$SCANNET_PURGE_PREPARED_AFTER_FAILED purge_on_skip=$SCANNET_PURGE_PREPARED_ON_SKIP"
...
if [[ "$DRY_RUN" != "1" ]]; then
  ...
  generate_args=(
    "$REPO_ROOT/evaluation/geometry/generate_scannet_must3r_profiles.py"
  ...
  echo "[scannet-must3r-benchmark:dry-run] would select scenes and generate MUSt3R profiles"
fi

if [[ ! -f "$PROFILE_LIST" ]]; then
  echo "[scannet-must3r-benchmark] missing profile list: $PROFILE_LIST" >&2
  exit 2
fi
...
link_sequence_inputs_for_eval() {
  ...
  local dst_seq="$must3r_root/scenes_sam2_must3r/$scene_id/sequence"
  if [[ "$DRY_RUN" == "1" ]]; then
...
cleanup_profile_artifacts() {
  local profile="$1"
  mapfile -t info < <(profile_info "$profile")
  safe_rm_generated_root "${info[3]}"
  safe_rm_generated_root "${info[4]}"
}
...
summarize_runs() {
  if [[ "$DRY_RUN" == "1" ]]; then
    echo "[scannet-must3r-benchmark:dry-run] would summarize $GEOMETRY_EVAL_GROUP"
    return 0
  fi
...
for profile in "${PROFILES[@]}"; do
  ...
  if (
    ...
    link_sequence_inputs_for_eval "$scene_id" "${info[4]}"
...
echo "[scannet-must3r-benchmark] completed=$completed skipped=$skipped failed=${#failed[@]}"
echo "[scannet-must3r-benchmark] summary=$REPO_ROOT/evaluation/outputs/geometry/$GEOMETRY_EVAL_GROUP/geometry_summary.md"

if (( ${#failed[@]} > 0 )); then
  printf '[scannet-must3r-benchmark] failed profiles:\n' >&2
  printf '  %s\n' "${failed[@]}" >&2
  exit 1
fi
