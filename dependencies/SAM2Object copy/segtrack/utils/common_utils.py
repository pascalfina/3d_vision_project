    ...
    def draw_masks_and_box_with_supervision(raw_image_path, mask_path, json_path, output_path):
        ...
        for raw_image_name in raw_image_name_list:
            ...
            if not os.path.exists(mask_npy_path):
                print(f"Skip frame {raw_image_name}: missing mask file {mask_npy_path}")
                continue
            ...
            if not os.path.exists(file_path):
                print(f"Skip frame {raw_image_name}: missing json file {file_path}")
                continue
            ...
            labels = [
                f"{instance_id}: {object_id_to_name.get(instance_id, '')}"
                for instance_id in all_object_ids
            ]
