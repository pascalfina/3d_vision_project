...
def get_similar_confidence_matrix_handle(seg_neighbors,
    ...
    for m in tqdm(range(view_num)):
        ...
        for i in range(seg_neighbors.shape[0]):
            # Support both adjacency rows (length S, values {0,1}/bool) and explicit neighbor-index arrays.
            raw_neighbors = np.asarray(seg_neighbors[i])
            if raw_neighbors.ndim == 1 and raw_neighbors.size == seg_num:
                seg_nei = np.nonzero(raw_neighbors)[0]
            else:
                seg_nei = raw_neighbors.reshape(-1).astype(np.int64, copy=False)
                seg_nei = seg_nei[(seg_nei >= 0) & (seg_nei < seg_num)]
...
def multiview_get_similar_confidence_matrix_handle(seg_neighbors,
    ...
    for m in tqdm(range(view_num)):
        ...
        if all_similiar:
            ...
            for i in range(seg_neighbors.shape[0]):
                ...
                if seg_nei.size == 0:
                    continue
                neighbors_labels = nonzero_seglabels[seg_nei]
                one_view_similar[i, seg_nei] = calcu_similar(
                    nonzero_seglabels[i],
                    neighbors_labels,
                    similar_metric=similar_metric,
                    thres_trunc=thres_trunc,
        ...
        if not np.isnan(m_distance).all():
            ...
            # distance_weights = 0.9 * (max_m_distance - m_distance) / (max_m_distance - min_m_distance) + 0.1
            denom = (max_m_distance - min_m_distance)
            if denom <= 1e-12:
                # All valid distances are (almost) identical in this view: use neutral weight.
                distance_weights = np.where(np.isnan(m_distance), 0.0, 1.0).astype(np.float32)
            else:
                distance_weights = 0.9 * np.power((max_m_distance - m_distance) / denom, 2) + 0.1
