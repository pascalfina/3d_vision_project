...
def inference_video_multi_ar(encoder, decoder, imgs, true_shape, mem_batches,
    ...
    if persistent_state is None:
        ...
        mem = persistent_state.get("mem")
        img_labels = dict(persistent_state.get("img_labels", {}))
        keyframes = set(persistent_state.get("keyframes", set()))
        id_offset = int(persistent_state.get("id_offset", 0))
    mem_batches = [0] + np.cumsum(mem_batches).tolist()
    pointmaps_0 = [None for _ in range(mem_batches[-1])]
    img_ids = [torch.tensor(id_offset + v) for v in range(nimgs)]
    ...
    if return_persistent_state:
        state_out = {
            ...
            "keyframes": keyframes,
            "id_offset": id_offset + nimgs,
        }
        if return_mem:
            return mem, pointmaps_0, state_out
        return pointmaps_0, state_out
    if return_mem:
