...
def get_reconstructed_scene(outdir, viser_server, should_save_glb, model, retrieval, device, verbose, image_size, amp,
                            ...
                            loaded_files="",
                            persistent_state=None, return_persistent_state=False,
                            ):
    """
    ...
    if execution_mode == "vidseq" or execution_mode == "vidslam":
        if execution_mode == "vidseq":
            ...
            if persistent_state is not None and persistent_state.get("scene_state") is not None:
                scene_state = persistent_state["scene_state"]
            else:
        ...
        result = must3r_inference_video(model, device, image_size, amp, image_list, max_bs, init_num_images=2, batch_num_views=1,
                                       ...
                                       verbose=verbose,
                                       persistent_state=persistent_state, return_persistent_state=return_persistent_state)
        if return_persistent_state:
            scene, state_out = result
            if state_out is not None:
                state_out["scene_state"] = scene_state
        else:
            scene = result
    else:
    ...
    if return_persistent_state:
        return scene, outfile, state_out
