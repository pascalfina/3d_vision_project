...
class MUSt3R_Retriever(Retriever):
    def __init__(self, *args, **kwargs):
        if not HAS_RETRIEVAL:
            raise ImportError("MUSt3R retrieval dependencies are not installed")
...
def must3r_inference(model, retrieval, device, image_size, amp,
    ...
    if is_sequence or retrieval is None:
        ...
        if not HAS_RETRIEVAL:
            raise ImportError("MUSt3R retrieval was requested but optional retrieval dependencies are missing")
        # run encoder
...
def must3r_inference_video(model, device, image_size, amp,
    ...
    with torch.autocast("cuda", dtype=dtype):
        result = inference_video_multi_ar(encoder, decoder, imgs, true_shape, mem_batches,
                                         ...
                                         return_persistent_state=return_persistent_state)
        if return_persistent_state:
            x_out, state_out = result
        else:
            x_out = result
            state_out = None
